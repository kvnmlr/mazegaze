...
In general, there are many possibilities to extend the game MazeGaze. In stead of control the menu by mouse you could implement a gaze bases control menu. So for example you have to look in different areas to choose different options in the menu. You also could create a collaborative team mode so that two players are in a team and other to players are in a opponent team and the teams have to play against each other. For example that means, if a player of the one team collect a bad power-up it just will be triggered on the members of the other team. Another idea would be to change the maze in play time. Like the game "Das Verrückte Labyrinth" you could destroy walls and build up other walls, but still it is a fair maze, so every can reach each point of the maze. \textbf{Calibration} \textbf{Improve Gaze Precision} \textbf{Queue} \textbf{fehlen noch hier}\\
1/2 page (Marco)\\
---\\
TOTAL: 0.75 page
